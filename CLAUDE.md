...
## Simulation Experiment Convention

The `data/` directory is for transient output (CSVs, plots, videos generated by `characterize_system.py` or similar tools). Files dropped directly into `data/` root are considered disposable and may be deleted at any time.

If a simulation experiment produces results worth keeping, create a subdirectory for it:

```
data/
  <experiment_name>/
    README.md        # what was done, why, key findings
    *.csv / *.png / *.mp4
```

The `README.md` should cover at minimum: the config used (or inline parameters if no TOML), the model file and its key parameters, what the experiment was testing, and the main result or observation. A few sentences is enough.
